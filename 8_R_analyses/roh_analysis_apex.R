...
roh20 <- read.csv(datapath('roh20X_full_ABO.csv'),header=T)
...
diet <- relevel(roh20$Apex.predator,ref='N')
diet <- factor(diet)
...
plot(log(roh20$N50),proprohs, xlab="log(N50)",ylab=bquote('log('*italic(P)[ROH]*')'),pch=19)
nroh.test <- cor.test(log(roh20$N50),proprohs,method='spearman')

r <- round(nroh.test$estimate,2)
text(11,-11,bquote(r == .(r)))

pval <- round(nroh.test$p.value,3)
text(11,-12,bquote(italic('p = ')*.(pval)))
...
tiff(filename=outpath("fig_num_rohs.tiff"),width=90,height=90,units="mm",res=1000, pointsize=6,
     compression = "lzw",type='cairo')
...
cols <- rev(c('firebrick2','forestgreen'))
...
#Proportion of ROHs
tiff(filename=outpath("fig_prop_rohs.tiff"),width=90,height=90,units="mm",res=1000, pointsize=6,
     compression = "lzw",type='cairo')
...
plot(1:2,mns,pch=19,xlim=c(0.5,2.5),xaxt='n',ylim=c(1.075*min(lower),0.95*max(upper)),
     ylab=bquote('log('*italic(P)[ROH]*')'%+-%'SE'),cex=2.5,xlab="",main='Red List Status')
axis(1,at=c(1:2),labels=c('Non-threatened','Threatened'),tick=F)
segments(1:2,lower,1:2,upper)
wd=0.05
segments(c(1:2)-wd,lower,c(1:2)+wd,lower)
...
for (i in 1:2){
  text(i,-14.1,bquote(italic('n=')*.(nsamp[i])))
}

text(0.6,-11.2,'A',cex=2)
...
plot(1:2,mns,pch=19,xlim=c(0.5,2.5),xaxt='n',ylim=c(1.075*min(lower),0.95*max(upper)),
     ylab=bquote('log('*italic(P)[ROH]*')'%+-%'SE'),cex=2.5,xlab="",main='Diet Category')
axis(1,at=c(1:2),labels=c('Non-carnivore','Carnivore'),tick=F)
segments(1:2,lower,1:2,upper)
wd=0.05
segments(c(1:2)-wd,lower,c(1:2)+wd,lower)
...
for (i in 1:3){
  text(i,-15.25,bquote(italic('n=')*.(nsamp[i])))
}

text(2.4,-11.1,'B',cex=2)

#Mass

pr.mass <- cor.test(data$proprohs,data$mass,method='spearman')
plot(data$mass,data$proprohs,pch=19,ylab=bquote('log('*italic(P)[ROH]*')'),main='Body Mass',
     xlab="Body Mass (z-score)")
...
text(1.75,-19.5,'C',cex=2)

#Latitude

pr.lat <- cor.test(data$proprohs,data$lat,method='spearman')
plot(data$lat,data$proprohs,pch=19,ylab=bquote('log('*italic(P)[ROH]*')'),main='Latitude',
     xlab="Latitude (z-score)")
